...
//! from the parent arrays (a maintained reverse index, never independent truth). Nothing is ever removed (a removed
...
impl std::fmt::Display for EditorGraphIndex {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EditorGraphIndex::Node(i) => write!(f, "p{i}"),
...
/// Where a reference sits — a VIEW synthesized from the layout table by
...
/// [`RefPositions`](but_graph::ref_layout::RefPositions).
#[derive(Debug, Clone, Default)]
pub(crate) struct EditorGraph {
    /// THE arena: `EditorGraphIndex::Node(i)` IS the commit-graph node index `i`. Commit ids are the payload
    ...
    /// [`RefLayout`](but_graph::ref_layout::RefLayout) it is ingested from (one word, both
    /// sides of the handoff): per STORED (unresolved) key, the reference groups standing on it.
    ...
    layout: HashMap<EditorGraphIndex, Vec<RefGroup>>,
    /// The derived children index, parallel to the arena: `children[p]` holds every
    /// `(child, slot)` edge naming node `p`, sorted. Maintained through
...
impl EditorGraph {
    ...
    pub(crate) fn adopt(arena: but_graph::CommitGraph) -> Self {
        ...
        for i in 0..arena.node_count() {
            for (slot, parent) in arena.parent_indices(i).into_iter().enumerate() {
                children[parent].push((EditorGraphIndex::Node(i), slot));
            }
        }
        Self {
            arena,
    ...
    /// Replace the node payload at `entry` with `step` — a pick decomposes into id and
    ...
    pub(crate) fn set_step(&mut self, entry: EditorGraphIndex, step: Step) {
        let EditorGraphIndex::Node(i) = entry else {
    ...
    pub(crate) fn node_ids(&self) -> impl Iterator<Item = EditorGraphIndex> + '_ {
    ...
    /// layout table (see [`RefPosition`]). `None` until placed.
    ...
    fn place(
        ...
        let groups = self.layout.entry(key).or_default();
    ...
    fn coalesce_groups(&mut self, key: EditorGraphIndex) {
        let Some(groups) = self.layout.get_mut(&key) else {
            return;
        };
        loop {
    ...
    /// children read, served by the maintained index.
    pub(crate) fn incoming_edges(&self, entry: EditorGraphIndex) -> &[Edge] {
        match entry {
            EditorGraphIndex::Node(i) => &self.children[i],
    ...
    pub(crate) fn redirect_children(&mut self, from: EditorGraphIndex, to: EditorGraphIndex) {
        ...
        let mut children: Vec<EditorGraphIndex> = self
            .incoming_edges(from)
            .iter()
            .map(|&(child, _)| child)
            .collect();
    ...
    fn update_parents<R>(
        ...
        let old = self.arena.parent_indices(i);
        let mut parents: Vec<EditorGraphIndex> =
            old.iter().copied().map(EditorGraphIndex::Node).collect();
        let result = f(&mut parents);
        let targets: Vec<usize> = parents
            .into_iter()
            ...
            .collect();
        for (slot, &parent) in old.iter().enumerate() {
            let edges = &mut self.children[parent];
            if let Ok(at) = edges.binary_search(&(child, slot)) {
                edges.remove(at);
            }
        }
        for (slot, &parent) in targets.iter().enumerate() {
            let edges = &mut self.children[parent];
            match edges.binary_search(&(child, slot)) {
                Err(at) => edges.insert(at, (child, slot)),
                Ok(_) => debug_assert!(false, "children index already names ({child}, {slot})"),
            }
        }
