...
impl<'meta, M: RefMetadata> Editor<'meta, M> {
    ...
    pub fn create_with_opts(
        ...
        let (graph, references, checkouts) = build_editor_graph(commit_graph, options)?;
        Ok(Self {
            graph,
