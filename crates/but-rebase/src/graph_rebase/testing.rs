...
impl TestingDot for EditorGraph {
    fn steps_dot(&self) -> String {
        ...
        for idx in self.node_ids().chain(self.ref_indices()) {
...
fn rendered_parents(graph: &EditorGraph, entry: EditorGraphIndex) -> Vec<EditorGraphIndex> {
    let groups = ref_groups(graph);
...
where
    F: FnMut(gix::ObjectId) -> Option<String>,
{
    let entries: HashSet<EditorGraphIndex> = graph.node_ids().chain(graph.ref_indices()).collect();
