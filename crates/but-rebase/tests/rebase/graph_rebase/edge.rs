//! These tests exercise the add_step, insert_edge and remove_edge operations.
...
fn adding_a_step_returns_a_selector_that_can_be_connected_into_the_graph() -> Result<()> {
    ...
    editor.insert_edge(c_selector, new_selector, 1)?;
    editor.insert_edge(new_selector, a_selector, 0)?;
...
#[test]
fn inserting_at_an_occupied_slot_shifts_existing_parents() -> Result<()> {
    ...
    // `c`'s only parent is `b` at slot 0; inserting `a` there makes `a` the first
    // parent and shifts `b` to the merge side.
    editor.insert_edge(c_selector, a_selector, 0)?;
...
fn adding_an_edge_that_introduces_a_cycle_causes_an_error() -> Result<()> {
    ...
    let err = editor
        .insert_edge(a_selector, c_selector, 1)
...
fn removing_an_existing_edge_returns_its_order_and_allows_readding_it() -> Result<()> {
    ...
    editor.insert_edge(b_selector, a_selector, 0)?;
