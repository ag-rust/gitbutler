...
use but_graph::{
    Graph, SegmentMetadata, Workspace,
...
│               └── ►:2[2]:origin/B →:1:
│                   └── ·70bde6b (⌂|🏘) ►A, ►A-empty-01, ►A-empty-02, ►A-empty-03
...
"#]]
    ...
    assert_eq!(
        ws.graph.entrypoint()?.commit().map(|c| c.id),
    ...
    assert!(
        ws.graph
            .tip_skip_empty(ws.graph.entrypoint()?.segment.id)
            .is_none(),
...
"#]]
    ...
    let head_baseline =
        Workspace::from_head(&repo, &*meta, project_meta(&*meta), standard_options())?
            .validated()?;
    let head_baseline_tree = graph_tree(&head_baseline.graph).to_string();
    ...
    .validated()?;
    let workspace_baseline_tree = graph_tree(&workspace_baseline.graph).to_string();
...
fn workspace_target_commit_and_extra_target_commit_can_overlap() -> anyhow::Result<()> {
    ...
    let baseline = Workspace::from_head(&repo, &*meta, project_meta(&*meta), standard_options())?
        .validated()?;
    let baseline_tree = graph_tree(&baseline.graph).to_string();
...
"#]]
    ...
    assert_eq!(
        ws.graph.partial_segments().count(),
        0,
...
"#]]
    ...
    .validated()?;
    assert_eq!(ws.graph.partial_segments().count(), 0);
    snapbox::assert_data_eq!(
        graph_tree(&ws.graph).to_string(),
        snapbox::str![[r#"
...
│           └── ·cbc6713 (⌂|🏘) ►dependent, ►on-top-of-dependent
...
"#]]
    ...
    for remote_ref in [
        ...
        assert_eq!(
            ws.graph.tip_skip_empty(remote_segment.id).map(|c| c.id),
...
│           └── ·a3b180e (⌂|🏘)
│               └── ►:2[2]:main <> origin/main →:3:
│                   ├── ·ce09734 (⌂|🏘|✓) ►base-peer, ►base-peer-1, ►base-peer-2, ►base-peer-3, ►base-peer-4, ►base-peer-5, ►base-peer-6, ►base-peer-7, ►base-peer-8, ►unapplied
│                   └── 🏁·fafd9d0 (⌂|🏘|✓)
└── ►:3[0]:origin/main →:2:
    └── →:2: (main →:3:)
...
"#]]
    ...
    let debug_graph = graph_tree(&ws.graph);
    let target_segment = ws
        .graph
...
│       ├── 📙►:4[1]:A <> origin/A →:6:
│       │   └── ►:3[2]:anon:
│       │       ├── ·081bae9 (⌂|🏘|✓) ►A-inside[📁wt-A-inside], ►A-outside[📁wt-A-outside]
│       │       └── 🏁·3183e43 (⌂|🏘|✓)
│       └── ►:1[1]:B[📁wt-B-inside]
│           └── ·3e01e28 (⌂|🏘)
│               └── →:3:
...
"#]]
    ...
    snapbox::assert_data_eq!(
        graph_workspace(&ws).to_string(),
        snapbox::str![[r#"
📕🏘️:0:gitbutler/workspace[🌳] <> ✓refs/remotes/origin/main on cb7021b
...
/// A canonical, `usize`-free structural fingerprint of a segment `Graph`: one sorted line per
...
fn build_cg_graph(
    ...
    but_graph::graph_from_repository(
        ...
        project_meta(meta),
        standard_options(),
        but_graph::walk::Overlay::default(),
    )?
...
fn cg_to_sg_ws_ref_no_ws_commit_stack_branch_on_same_commit() -> anyhow::Result<()> {
    ...
    let (repo, mut meta) = read_only_in_memory_scenario("ws/just-init-with-branches")?;
    add_stack_with_segments(&mut meta, 0, "A", StackState::InWorkspace, &[]);
    let flip = but_graph::workspace_from_repository(
        &repo,
        &*meta,
        None,
        None,
    ...
    .expect("managed");
    snapbox::assert_data_eq!(
        graph_workspace(&flip).to_string(),
        snapbox::str![[r#"
📕🏘️⚠️:1:gitbutler/workspace <> ✓!
└── ≡📙:0:A {0}
    └── 📙:0:A
...
/// (`from_tip`) — exercising the entrypoint segment split.
...
fn assert_cg_to_sg_projects(repo: &gix::Repository, meta: &impl RefMetadata) -> anyhow::Result<()> {
    let real_ws =
        Workspace::from_head(repo, meta, project_meta(meta), standard_options())?.validated()?;
    let built_ws = but_graph::workspace_from_repository(
        repo,
        meta,
        None,
        None,
