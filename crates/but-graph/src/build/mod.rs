...
//! build was entered (managed workspace, non-managed checkout, explicit seeds, overlays).
//!
//! The build is phased (gather-then-build), one file per phase: [`facts`] (pure boundary/shape
//! reads) → [`plan`] (the chain plan, decided entirely as data) → [`materialize`] (the
//! repository-flavored decisions feeding [`segment_data`], whose segments install one-to-one
//! into segments), with [`chains`] and [`remotes`] holding shared structure decisions. Entry
//! points and shared helpers live here.
...
/// [`graph_from_repository`] with the workspace projection applied. `None` on the same
/// non-managed fall-through.
...
/// like a fresh walk, with the enrichment (refs, target, metadata) read from `repo`/`meta`
/// amended by `overlay` — a default overlay reads the CURRENT state (a materialized rebase),
/// a rebase's overlay serves its pending ref edits and entrypoint from memory (a preview).
/// Dispatches like [`Graph::from_head`](crate::Graph::from_head): managed with `HEAD` (or the
/// overlay entrypoint) as entrypoint when the workspace ref resolves to a commit in `cg`
/// (falling through to non-managed when the entrypoint lands outside the workspace),
/// non-managed otherwise. `None` when the entrypoint is unborn or unresolvable.
pub fn workspace_from_commit_graph<T: but_core::RefMetadata>(
    ...
    if let Some(ws_commit) = ws_commit {
        ...
        // to the non-managed view. The carried commit graph is reclaimed as-is: the non-managed
        // pass overwrites everything the managed attempt touched (refs are re-refreshed from a
        // fresh mapping, the layout is re-authored), and the extra managed-ws-commit mark on the
        // workspace tip is only ever read for the entrypoint, which a fall-through proves is a
        // different commit.
        if graph.entrypoint.is_some() {
            return graph.into_workspace().map(Some);
        }
        cg = graph.commit_graph;
    }
...
/// [`Graph::from_head`](crate::Graph::from_head). `overlay` refs and metadata are served
/// from memory (the flip counterpart of [`Graph::redo_traversal`](crate::Graph::redo_traversal)).
...
/// [`Graph::from_seeds`](crate::Graph::from_seeds). The tips'
...
pub(crate) fn graph_from_repository_seeds<T: but_core::RefMetadata>(
    ...
    // (Tips-built graphs read their seeds' roles straight from the carried commit graph.)
    if graph
        .seeds()
        .iter()
...
pub(super) struct BuildInputs<'a> {
    ...
    pub managed: bool,
    /// Which worktree (if any) checks out each ref, keyed by ref name — the main worktree `[🌳]`
...
fn assemble_managed<T: but_core::RefMetadata>(
    ...
    // workspace tip, the ref layout's ws-commit slot reads the (checkout) entrypoint.
    ...
    let b = BuildInputs {
        cg: &cg,
        workspace_commit: ws_commit,
        entrypoint,
        entrypoint_ref: entrypoint_ref.as_ref(),
        target: inputs.target,
        remote_tracking: &inputs.remote_tracking,
        symbolic_remotes: &inputs.symbolic_remotes,
        stack_branches: Some(&stack_branches),
        managed: true,
        project_meta: &project_meta,
        options: &options,
    };
    let (f, plan, mut layout) = plan::gather_and_plan(&b, overlay_meta);
    let (mut graph, mut segment_data) =
        graph_from_commit_graph(&b, overlay_meta, f, &plan, &layout);
    // A checked-out branch outside the managed workspace can still carry a persisted
    // single-branch ordering; the workspace itself never does (its orders are keyed by
    // the workspace ref, which has no persisted ordering).
    apply_ad_hoc_orders(
        &mut graph,
        &mut segment_data,
        &cg,
    ...
    let positions = ref_positions::derive_ref_positions(&segment_data, &cg)?;
    layout.positions = Some(positions);
    cg.layout = Some(layout);
    segment_data::enrich(&mut segment_data, overlay_meta, &inputs.worktree_by_branch);
    segment_data::render_segment_graph(&mut graph, segment_data);
    graph.commit_graph = cg;
...
/// Apply the persisted single-branch orderings that share the entrypoint's tip: split the
/// same-tip ref bundle into empty stack segments. A no-op unless the entrypoint ref carries such
/// an order. Rewrites the authored segments only — the assembler's final render installs them.
#[tracing::instrument(level = "trace", skip_all)]
fn apply_ad_hoc_orders<T: but_core::RefMetadata>(
    graph: &mut crate::Graph,
    segment_data: &mut segment_data::SegmentData,
    cg: &CommitGraph,
    overlay_repo: &OverlayRepo<'_>,
    overlay_meta: &OverlayMetadata<'_, T>,
) -> anyhow::Result<()> {
    graph.ad_hoc_branch_stack_upgrades(overlay_repo, overlay_meta)?;
    if graph.ad_hoc_branch_stack_orders.is_empty() {
        return Ok(());
    }
    segment_data::replay_ad_hoc(
        segment_data,
        cg,
        &graph.ad_hoc_branch_stack_orders,
        graph.entrypoint_ref.as_ref(),
    );
    Ok(())
}
