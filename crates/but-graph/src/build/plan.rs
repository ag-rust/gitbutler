...
use crate::ref_layout::{Chain, GroupPlacement, LayoutGroup, NamingRef, RefLayout};
...
pub(super) fn gather_and_plan<T: but_core::RefMetadata>(
    ...
    let ws_lower_bound = super::chains::effective_lower_bound(
        b.cg,
        b.workspace_commit,
        b.target,
        b.project_meta,
    ...
    let plan = chain_plan(b, &f, ws_lower_bound, meta);
    // The ref placement table: stored on the commit graph and consumed by the chain-structure
    // pass; the debug assert proves it round-trips the plan it came from.
    let layout = author_layout(&plan);
    debug_assert_layout_matches_plan(&layout, &plan);
    (f, plan, layout)
}
...
pub(super) struct RefGroup {
    ...
    /// How the group's non-naming members land, in metadata order. The remaining members
    /// either name the anchor or already name another segment.
...
pub(super) struct ChainPlan {
    ...
    /// non-bottom naming ref is displaced, the remote creators link its floated empty instead).
    ...
    /// segment). The remote/target passes key their decisions on these — they evaluate the
    /// pre-chain view.
    ...
    /// pointing behind/at an anonymous owner names it; the target and explicit seeds likewise).
...
///   float) so their stacks' branches form their own chains (`anonymize_shared_bases`),
...
fn thread_ref_groups(
    ...
    for (li, list) in lists.iter().enumerate() {
        ...
        while i < list.len() {
            ...
            // gate (the group naming ref included — it names the anchor, not an empty). ──
            ...
            plan.ref_order[li].push(RefGroup { commit, placement });
        }
    }
}
...
/// Project the plan's ref decisions into the STORED table shape (see `crate::ref_layout`):
...
/// `debug_assert_layout_matches_plan` proves the shape carries the plan losslessly on
/// every build.
pub(super) fn author_layout(plan: &ChainPlan) -> RefLayout {
    let mut layout = RefLayout::default();
    for (li, chain) in plan.ref_order.iter().enumerate() {
        ...
        for group in chain {
            let naming_ref =
                plan.group_names
                    .get(&(li, group.commit))
                    .map(|(name, clear_remote)| NamingRef {
                        name: name.clone(),
                        clear_remote: *clear_remote,
                    });
            let groups = layout.at_commit.entry(group.commit).or_default();
...
/// Debug assert: reconstruct the plan's ref decisions FROM the stored table and
/// require exact equality — chain count and threading order, per-group empties and role,
/// naming refs (with their displacement flag), and anonymous bases. Non-trivial precisely where the shape
...
pub(super) fn debug_assert_layout_matches_plan(layout: &RefLayout, plan: &ChainPlan) {
    ...
    assert_eq!(
        layout.chains.len(),
        plan.ref_order.len(),
        "layout chains must mirror the plan's stack lists"
    );
    let mut group_names = HashMap::new();
    for (li, stored) in layout.chains.iter().enumerate() {
        let chain: Vec<RefGroup> = stored
            ...
            .map(|&(commit, gi)| {
                ...
                if let Some(naming_ref) = &group.naming_ref {
                    group_names.insert(
                        (li, commit),
                        (naming_ref.name.clone(), naming_ref.clear_remote),
                    );
                }
                RefGroup {
                    commit,
    ...
    let mut anonymous_bases: Vec<_> = plan.anonymous_bases.iter().copied().collect();
    anonymous_bases.sort();
    assert_eq!(
        layout.anonymous_bases, anonymous_bases,
        "the table's demotions must match the plan's"
    );
    let anchored: usize = layout.chains.iter().map(|c| c.anchors.len()).sum();
    let stored: usize = layout.at_commit.values().map(Vec::len).sum();
