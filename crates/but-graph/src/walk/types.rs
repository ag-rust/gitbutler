...
use crate::{CommitFlags, CommitIndex};
...
pub struct TopoWalk {
    /// The segment we
    next: VecDeque<(usize, Option<CommitIndex>)>,
    ...
    seen_empty_segments: BTreeSet<usize>,
    ...
    pub leafs: Option<Vec<usize>>,
}

/// Lifecycle
impl TopoWalk {
    /// Start a walk at `segment_id`, possibly only from `commit`.
...
impl TopoWalk {
    ...
    pub fn next(&mut self, graph: &crate::Graph) -> Option<(usize, Range<CommitIndex>)> {
    ...
    fn next_inner(&mut self, graph: &crate::Graph) -> Option<(usize, Range<CommitIndex>)> {
        ...
        for edge in graph.edges_directed(segment, self.direction) {
            count += 1;
            // Derived endpoint indices: a present source id is the source's last commit,
            // a present destination id is the target's first.
            let src_cidx = edge
                .connection
                .src_id()
                .and_then(|_| graph[edge.source].commits.len().checked_sub(1));
            let dst_cidx = edge.connection.dst_id().map(|_| 0);
            match self.direction {
                Direction::Outgoing => {
                    if src_cidx.is_some_and(|src_cidx| !available_range.contains(&src_cidx)) {
                        continue;
                    }
                    self.next.push_back((edge.target, dst_cidx));
                }
                Direction::Incoming => {
                    if dst_cidx.is_some_and(|dst_cidx| !available_range.contains(&dst_cidx)) {
                        continue;
                    }
                    self.next
                        .push_back((edge.source, src_cidx.map(|cidx| cidx + 1)));
                }
            }
        }
    ...
    fn select_range(
        ...
        match first_commit_index {
            None => {
                debug_assert!(
                    graph[segment_id].commits.is_empty(),
                    "BUG: we always assure that we set the commit index if a segment has commits: {segment_id:?}"
                );
                if !self.seen_empty_segments.insert(segment_id) {
                    return None;
                }
                Some(0..0)
            }
            Some(start_commit_idx) => {
                let segment = &graph[segment_id];
