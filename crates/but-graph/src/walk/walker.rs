...
//! `initial_seeds_*`, `find`), and naming/metadata decisions delegate to
...
pub(crate) struct WalkOutcome {
    ...
    /// The normalized traversal seed_segments, like the raw graph's `traversal_tips`.
    pub seeds: Vec<Seed>,
}
...
impl Segs {
    ...
    fn sync_seeds(&mut self, seed_segments: &[Segment], seed_seg: &mut Vec<usize>) {
...
pub(crate) fn traverse<T: RefMetadata>(
    ...
    let mut seed_segments: Vec<Segment> = Vec::new();
    ...
    prioritize_and_assure_ws_ownership(
        &mut next,
        &mut seed_segments,
        &mut ep_seed,
    ...
    while let Some((info, mut propagated_flags, instr, mut limit)) = next.pop_front() {
        ...
        if seen.contains(&id) {
            // Re-encounter: record the edge, run the segment-ownership choreography
            // (workspace swap, mid-segment split), merge flags, propagate downward,
            // adjust queued tips.
        ...
        let seg_for_id = if let Some(ix) = instr.seed {
            ...
            let maybe_name = super::utils::disambiguate_refs_by_branch_metadata_with_lookup(
                (&refs_by_id, id),
                meta,
            );
    ...
    Ok(WalkOutcome {
        ...
        seeds: initial_seeds.seeds.clone(),
    })
}
...
fn queue_initial_seeds<T: RefMetadata>(
    ...
    let mut pending_integrated_seeds = BTreeMap::<gix::refs::FullName, PendingSeed>::new();

    #[allow(clippy::too_many_arguments)]
    fn queue_pending(
        next: &mut Queue,
        seed_segments: &[Segment],
        pending: PendingSeed,
    ...
    for seed in &initial_seeds.seeds {
        ...
        if let SeedRole::TargetRemote = &seed.role {
                ...
                .cloned()
            {
                let Some((_local_seed, local_goal)) = target_local_seeds.get(&target_ref).copied()
                    ...
                    pending_integrated_seeds.insert(target_ref, pending);
                    continue;
                };
                queue_pending(
                    next,
        ...
        let (flags, limit) = match &seed.role {
            ...
            SeedRole::TargetLocal { local_ref_name } => {
                let has_remote_link = seed_segments[seg]
                    .ref_name()
                ...
                {
                    target_local_seeds
                        .insert(target_ref.clone(), (has_remote_link.then_some(seg), goal));
                }
        ...
        let seed_info = find(commit_graph, repo.for_find_only(), seed.id, buf)?;
        let item: QueueItem = (
            seed_info,
            flags,
            Instruction {
        ...
        let pending_before_current = paired_target_ref.as_ref().and_then(|target_ref| {
            pending_integrated_seeds
                .get(target_ref)
                .is_some_and(|pending| pending.id == seed.id)
                .then(|| pending_integrated_seeds.remove(target_ref))
                .flatten()
        });
        ...
        if let Some(target_ref) = paired_target_ref
            && let Some(pending) = pending_integrated_seeds.remove(&target_ref)
        {
...
fn prioritize_and_assure_ws_ownership<T: RefMetadata>(
    next: &mut Queue,
    seed_segments: &mut Vec<Segment>,
    ep_seed: &mut Option<usize>,
    (ws_tips, repo, meta): (
